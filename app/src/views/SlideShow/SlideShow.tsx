...
const SlideShow: React.FC = () => {
  ...
  const memoryCorrectSound = new Audio(
    `${config.SOUND_PATH}Memory correct.mp3`
  );
  const memoryWrongSound = new Audio(`${config.SOUND_PATH}Memory wrong.mp3`);
  const memoryAllCorrectSound = new Audio(
    `${config.SOUND_PATH}Memory all correct.wav`
  );
  ...
  const baseSlides: BaseSlides[] = [
    ...
    {
      slide: `${config.SLIDE_PATH}phase1/Slide9.JPG`,
      children: <></>,
      transit: "SHOP",
    },
