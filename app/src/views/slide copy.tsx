...
const SlideShow: React.FC = () => {
  ...
  const {
    ...
    waitTimeout,
    waitKeyPress,
  } = useSlideHandlers(slideNumber, trialNumber, incrementSlideIndex);
  ...
  const renderSlide = () => {
    ...
    const renderSlideString = (slidePath: string) =>
      slidePath.replace(
        /{(\w+)}/g,
        (_, group) => store.data.survey[group as keyof typeof store.data.survey]
      );

    if (typeof taskInfo === "string") {
      return {
        slide: `${SLIDE_PATH}${taskInfo}.JPG`,
        execute: () => setButtonVisible(true),
      };
    } else {
      if (taskInfo.showIf) showIf(taskInfo.showIf!);
      const slidePath = renderSlideString(taskInfo.slidePath!) ?? "";

      switch (taskInfo.type) {
        case "VAS":
          return vas(
            slidePath,
            taskInfo.variableName!,
            taskInfo.minLabel!,
            taskInfo.maxLabel!
          );
        ...
        case "contingency":
          return contingencySlide(taskInfo.index!);
        case "quiz":
          return quizSlide();
        default:
          return {
            slide: `${SLIDE_PATH}Slide${slideNumber}.JPG`,
            execute: () => waitKeyPress(),
          };
      }
    }
  };
  ...
  useEffect(() => {
    const { slide, execute } = renderSlide();
    setSlide(slide);
    execute();
  }, [slideNumber, trialNumber]);

  const setSlide = (slidePath: string) => {
    const slide = new Image();
    slide.src = slidePath;
    document.body.style.backgroundImage = `url(${slide.src})`;
  };
  ...
  return (
    <div>
      ...
      >
        <Button onClick={incrementSlideIndex}>Next</Button>
        <Button onClick={decrementSlideIndex}>Previous</Button>
      </div>
    </div>
  );
};
