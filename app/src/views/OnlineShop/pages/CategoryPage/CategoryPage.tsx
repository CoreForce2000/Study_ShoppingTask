...
import {
  selectClickedItemTiles,
  selectShuffledItemsByCategory,
  selectItemsByCategory,
  setShuffledItems,
  setItemTileClicked,
  Product,
  addItemToClickedItems,
  selectBudget,
...
const CategoryPage: React.FC<CategoryPageProps> = ({ category }) => {
  ...
  const clickedTiles = useSelector((state: RootState) =>
    selectClickedItemTiles(state, category)
  );
  const budget = useSelector((state: RootState) => selectBudget(state));
  const allItems = useSelector((state: RootState) =>
    selectItemsByCategory(state, category)
  );
  let shuffledItems = useSelector((state: RootState) =>
    selectShuffledItemsByCategory(state, category)
  );
  ...
  const onItemTileClick = (index: number, item: Product | null) => {
    ...
    if (item) {
      ...
      const currentItem: Product =
        shuffledItems[clickedTiles.length % shuffledItems.length];
      ...
      if (clickedTiles.length < shuffledItems.length - 1) {
        const nextItem: Product =
          shuffledItems[(clickedTiles.length + 1) % shuffledItems.length];
